...
| KAI-02 | Void Seeker leaves Pass Focus enabled while a Deflect payment is incomplete. | T3 | Accepted | Target-selection mode blocks Pass Focus and Pass Turn until the player submits a payable selection or cancels. | Validated in full Kai'Sa deck acceptance. |
| KAI-03 | Daughter of the Void uses a non-canonical image printing. | T5 | Accepted | Catalog selection now prefers the regular metadata printing over alternate, overnumbered, and signature variants; Daughter resolves to `OGN-247/298`. | Validated in full Kai'Sa deck acceptance. |
| KAI-04 | Daughter of the Void Add-resource ability is not available. | T5 | Accepted | The shared board-action menu now includes the Legend zone; the existing immediate resource primitive handles Reaction Add without opening the Chain. | Validated in full Kai'Sa deck acceptance. |
| KAI-05 | A unit preview does not disclose temporary keyword/text changes such as Cleave's Assault. | T5 | Accepted | Projection includes current temporary keywords/modifiers with their duration. | Validated in full Kai'Sa deck acceptance. |
...
| KAI-08 | Accelerate action wording is unclear. | T5 | Accepted | Projected Accelerate label follows `Play accelerated <card> to <location>`. | Validated in full Kai'Sa deck acceptance. |
...
| KAI-12 | Battlefield preview does not suit battlefield-specific information and interactions. | T5 | Accepted | Battlefield details use a dedicated landscape dialog with the printed art, full rules text, and current controlled/contested status, reachable from both the battlefield info control and a Battlefield card in the Chain. | Validated in full Kai'Sa deck acceptance. |
...
| KAI-15 | Falling Star/Icathian Rain target flow is unclear and Deflect does not add required Power. | T2 + T3 | Accepted | Repeated assignments preserve order and duplicates; Deflect Power is counted once per selected assignment before play. | Validated in full Kai'Sa deck acceptance. |
| KAI-16 | Time Warp exists in both Trash and Banishment. | T4 | Accepted | A resolving spell goes to Trash only when no effect has moved it to another zone. | Validated in full Kai'Sa deck acceptance. |
...
| KAI-19 | A combat conquer can clear battlefield and Unit conquer triggers before they enter the Chain. | T6 | Accepted | Combat completion clears its own state before it scores, preserving queued conquer triggers. | Validated in full Kai'Sa deck acceptance. |
| KAI-20 | Repeated match polling spends about 630 ms reading the event log. | T7 | Accepted | Event logs are cached per game and invalidated after an event append. | Validated in full Kai'Sa deck acceptance. |
...
| KAI-24 | Simultaneous sideboard submissions reject one player with “Between-games state has changed.” | T9 | Accepted | A lost optimistic-concurrency write re-reads and merges against the same between-games ID once; a completed/replaced intermission remains rejected. | Validated in full Kai'Sa deck acceptance. |
