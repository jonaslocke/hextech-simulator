...
test("characterizes the separated showdown timing kernel", async () => {
  ...
  const showdownSchema =
    stateSource.match(
      /showdown:\s*z\s*\.\s*object\(\{[\s\S]*?\}\)\s*\.nullable\(\)/,
    )?.[0] ?? "";

  assert.match(actionsSource, /startCombat/);
  assert.match(
    stateSource,
    /showdown:\s*z\s*\.\s*object\(\{[\s\S]*?focusPlayerId:/,
  );
  assert.doesNotMatch(showdownSchema, /priorityPlayerId/);
  assert.match(
    stateSource,
    /chain:\s*z\s*\.\s*object\(\{[\s\S]*?priorityPlayerId:/,
  );
  assert.match(stateSource, /combat:\s*z\s*\.\s*object/);
...
test("records the controlling showdown rules and non-combat interpretation", async () => {
  const ledger = await readFile(
    "docs/showdown-rules-decision-ledger.md",
    "utf8",
  );
