...
if (!process.argv.includes("--confirm")) {
  throw new Error("Refusing to approve the OGN opponent-Hand selection family without --confirm.");
}
...
const MODELS: Record<string, Assignment[]> = {
  "OGN-156": [
    ...
    { family: "selector", primitiveId: "selector.card", parameters: { zone: "hand", cardType: "nonUnit", owner: "opponent", minimumCount: 1, maximumCount: 1, deferred: true, revealZone: true, selectionKey: "cardToRecycle" } },
    { family: "action", primitiveId: "action.recycle_cards", parameters: { target: "card", selectionKey: "cardToRecycle" } },
  ],
  "OGN-192": [
    { family: "trigger", primitiveId: "trigger.on_play", parameters: { actor: "controller", subject: "source" } },
    { family: "selector", primitiveId: "selector.card", parameters: { zone: "hand", cardType: "any", owner: "opponent", minimumCount: 1, maximumCount: 1, deferred: true, revealZone: true, selectionKey: "cardToDiscard" } },
    { family: "action", primitiveId: "action.discard_cards", parameters: { player: "selectedCardOwner", count: 1, selectionKey: "cardToDiscard" } },
  ],
};
...
try {
  ...
  for (const [cardCode, assignments] of Object.entries(MODELS)) {
    ...
    const document = await publishCanonicalCard(
      db,
      {
        adminNotes: "OGN M2 opponent-Hand reveal and selected-card disposition family.",
        card,
