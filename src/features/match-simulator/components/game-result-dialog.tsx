...
import { ArrowRight, Crown, Sparkles, Timer, Trophy } from "lucide-react";
...
export function GameResultDialog({
  ...
  const game = projection.currentGame;

  useEffect(() => {
    setSecondsRemaining(COUNTDOWN_SECONDS);
  }, [projection.betweenGames?.id]);
  ...
  if (projection.status !== "between_games" || !game.winnerPlayerId) {
    return null;
  }

  const viewerWon = game.winnerPlayerId === projection.viewerPlayerId;
  const winner = game.players.find(
    (player) => player.playerId === game.winnerPlayerId,
  );
        ...
        >
          ...
          <header className="relative justify-items-center gap-3 grid pt-2">
            ...
            <div>
              ...
              >
                {winnerName} won Game {game.gameNumber}. The match continues.
              </p>
            </div>
          </header>
          ...
          <div className="relative flex justify-between items-center gap-3 bg-slate-900/50 px-3 py-2 border border-white/10 rounded-xl text-slate-400 text-xs">
            ...
            <span className="inline-flex items-center gap-1 text-amber-100 tabular-nums">
...
function getPlayerDisplayName(
  ...
  return displayName || player?.playerId || fallback;
}
