...
export function CardInspector({ card }: { card: SideboardingCardView | null }) {
  return (
    <aside className="hidden min-h-0 flex-col rounded-md border border-white/10 bg-slate-950/75 p-3 text-slate-100 xl:flex">
      <CardFace card={card} />
      {card && (
        <div className="mt-3 min-h-0 overflow-y-auto">
          <h2 className="font-semibold leading-tight">{card.name}</h2>
          <p className="mt-1 text-slate-500 text-xs">
            {[card.type, card.supertype, card.domains.join("/")].filter(Boolean).join(" / ")}
          </p>
          {card.rulesText.trim() && (
