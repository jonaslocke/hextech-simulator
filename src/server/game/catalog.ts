...
export function buildDeckSnapshot(
  ...
  sourceCatalog?: Pick<CardCatalog, "cards" | "byName" | "canonicalByName">,
): DeckSnapshot {
  ...
  const tokenCards = catalog.cards.filter(
    (card) => card.classification.supertype === "Token",
  );
  const digest = createHash("sha256")
    .update(JSON.stringify({
      cards: [...cards].sort((left, right) => left.cardCode.localeCompare(right.cardCode)),
      tokenCards: [...tokenCards].sort((left, right) => left.public_code.localeCompare(right.public_code)),
    }))
    .digest("hex");
  return deckSnapshotSchema.parse({ sourceText, catalogDigest: digest, entries, cards, tokenCards });
}

function resolveEntries(
  sourceText: string,
  catalog: Pick<CardCatalog, "cards" | "byName" | "canonicalByName">,
) {
  const issues: string[] = [];
