...
const CATALOG_SEEDS: Record<string, PrimitiveCatalogSeed> = {
  ...
  "trigger.on_kill": primitiveSeed({
    id: "trigger.on_kill",
    family: "trigger",
    name: "Kill trigger",
    description: "Creates an effect when the controller kills a Unit by the required method.",
    parameters: [
      optional("method", "string", "The required kill source.", ["spell"]),
      optional("subject", "string", "An optional killed-unit state and ownership filter.", ["stunned_enemy_unit"]),
    ],
    listensToEvents: ["unit.killed"],
    engineSupport: supported("Damage resolution emits a separate kill-attribution event after lethal cleanup."),
  }),
  "trigger.look_or_reveal_source": primitiveSeed({
    id: "trigger.look_or_reveal_source",
    family: "trigger",
    name: "Look-or-reveal source trigger",
    description: "Creates an effect when the source is looked at or revealed from a hidden zone.",
    listensToEvents: ["card.revealed"],
    engineSupport: supported("Private look and public reveal events both retain the source identity for its controller."),
  }),
  ...
  "selector.card": primitiveSeed({
    ...
    parameters: [
      ...
      optional("maximumEnergyBelowControllerPoints", "boolean", "Whether printed Energy must be lower than the controller's current points."),
      ...
      optional("requiredBehaviorId", "string", "A behavior or keyword the selected card must have."),
      optional("requiredPaymentDomain", "string", "A Power domain that must be payable for the selection to be offered."),
      optional("requiredPaymentAmount", "number", "The Power amount that must be payable."),
      optional("selectionKey", "string", "Stable key used to route this selection."),
      optional("deferred", "boolean", "Whether selection is made during effect resolution rather than while playing the card."),
  ...
  "action.play_selected_card": primitiveSeed({
    ...
    family: "action",
    name: "Effect-driven card play",
    description: "Plays a selected card from an effect-authorized zone with a declared cost and destination treatment.",
    parameters: [
      optional("source", "string", "Whether the behavior source itself is played.", ["self"]),
      optional("sourceSelectionKey", "string", "Selector key containing the card to play."),
      optional("selectionKey", "string", "Key storing a Unit destination or the effect choice."),
      required("costMode", "string", "How the play cost is paid.", ["ignoreAll", "powerOnly", "oneAnyPower"]),
      optional("destination", "string", "A fixed Unit destination.", ["sourceBattlefield"]),
      optional("afterResolution", "string", "A Spell's post-resolution zone.", ["recycle"]),
    ],
    engineSupport: supported("The effect verifies the source zone and payment, records the play, uses normal Unit destination legality, and puts played Spells on the Chain."),
  }),
  "action.pay_optional_resource": primitiveSeed({
    id: "action.pay_optional_resource",
    family: "action",
    name: "Optional resolution resource payment",
    description: "Offers a validated resource payment while an effect resolves and records whether it was paid.",
    parameters: [
  ...
  "action.pay_resource": primitiveSeed({
    ...
    family: "action",
    name: "Resolution resource payment",
    description: "Pays a required resource cost during effect resolution after legality was established.",
    parameters: [
      required("resource", "resource", "The resource kind.", ["energy", "power"]),
      required("amount", "number", "The amount paid."),
      optional("domain", "string", "The required Power domain; omit it to accept Power of any domain."),
  ...
  "action.pay_optional_buff": primitiveSeed({
    ...
    family: "action",
    name: "Optional resolution Buff payment",
    description: "Lets the controller select and spend one Buff while an effect resolves.",
    parameters: [required("selectionKey", "string", "Key recording the selected Buff source.")],
    engineSupport: supported("The choice exposes only friendly buffed Units and revalidates the selected Buff before spending it."),
  }),
  "action.pay_optional_exhaust": primitiveSeed({
    id: "action.pay_optional_exhaust",
    family: "action",
    name: "Optional resolution exhaust payment",
    description: "Offers exhaustion of a ready source or Legend as an optional resolving cost.",
    parameters: [
      required("target", "target", "The card exhausted.", ["source", "legend"]),
      required("selectionKey", "string", "Key recording accept or decline."),
  ...
  "condition.event_subject_characteristic": primitiveSeed({
    id: "condition.event_subject_characteristic",
    family: "condition",
    name: "Event subject characteristic",
    description: "Guards a clause by a printed characteristic of the event subject.",
    parameters: [
      optional("tag", "string", "A required card tag."),
      optional("minimumMight", "number", "A required current Unit Might."),
    ],
    engineSupport: supported("The runtime evaluates the event subject against its immutable deck-snapshot definition."),
  }),
  ...
  "cost.discard_cards": primitiveSeed({
    id: "cost.discard_cards",
    family: "cost",
    name: "Discard cards activation cost",
    description: "Discards selected cards from hand while declaring an activated ability.",
    parameters: [
      required("count", "number", "The number of cards discarded."),
      required("selectionKey", "string", "Selector key containing the cards discarded."),
    ],
    engineSupport: supported("Activation projection requires enough hand cards and declaration atomically moves the locked selections to Trash."),
  }),
  "replacement.optional_recall_on_death": primitiveSeed({
    id: "replacement.optional_recall_on_death",
    family: "replacement",
    name: "Optional paid death replacement",
    description: "Offers a validated payment to heal, exhaust, and recall a Unit instead of a pending death.",
    parameters: [
      optional("selectionKey", "string", "Selector key containing a protected Unit."),
      required("target", "target", "The protected Unit scope.", ["selectedUnit", "friendlyBuffedUnit"]),
      required("resource", "resource", "The payment resource.", ["energy", "power"]),
      required("amount", "number", "The payment amount."),
      optional("domain", "string", "The required Power domain."),
      optional("exhaustSource", "boolean", "Whether the replacement source exhausts."),
      optional("spendTargetBuff", "boolean", "Whether the protected Unit spends its Buff."),
      required("duration", "duration", "How long a registered one-shot replacement remains available."),
    ],
    engineSupport: supported("The death pipeline suppresses lethal cleanup while the owning player decides, then revalidates payment and resumes queued simultaneous deaths."),
  }),
  ...
  "modifier.active_in_zone": primitiveSeed({
